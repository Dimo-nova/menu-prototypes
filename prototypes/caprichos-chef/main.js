...
const dailyMenuConfig = {
  ...
  segundos: ["pimientos_padron", "trigueros_alioli", "croquetas_caseras"],
};
...
const buildDishCard = (dish) => {
  ...
  card.className = `dish-card ${!hasImage ? "dish-card-no-image" : ""} ${hasVariants ? "dish-card-variants" : ""}`;
  if (!hasVariants) {
  ...
  if (!hasImage && dish.badge && !hasVariants) {
  ...
  if (hasVariants) {
    ...
    topRow.className = "dish-card-top-row";
    if (imageContainer) {
...
const openDishModal = (dish) => {
  ...
  if (hasVariants) {
    variantsHtml = `
      <div class="detail-row">
        <span class="detail-label"><i class="fa-solid fa-list"></i> Opciones de carne</span>
  ...
  dom.dishModalBody.innerHTML = `
    ...
    ${!hasVariants ? `<span class="modal-price">${dish.price}</span>` : ""}
