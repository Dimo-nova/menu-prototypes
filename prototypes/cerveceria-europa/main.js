  ...
  const dishes = raw.categories.flatMap((category) =>
    category.dishes.map((dish) => {
      ...
      return {
        ...
        title: dish.name,
        price: dish.price || "",
        priceOptions: Array.isArray(dish.priceOptions)
...
const buildDishCard = (dish) => {
  ...
  if (!dish.priceOptions || dish.priceOptions.length === 0) {
  ...
  if (dish.priceOptions && dish.priceOptions.length > 0) {
    ...
    dish.priceOptions.forEach((option) => {
      if (!option.price) return;
      const item = document.createElement("div");
      item.className = "card-price-option";
      item.innerHTML = `
        <span class="card-price-label">${option.label}</span>
        <span class="card-price-value">${option.price}</span>
      `;
...
const openDishModal = (dish) => {
  ...
  const modalPriceMarkup = (dish.priceOptions && dish.priceOptions.length > 0)
    ? `
      <div class="modal-prices">
        ${dish.priceOptions
          ...
          .map(
            (option) => `
              <div class="modal-price-option">
                <span class="modal-price-label">${option.label}</span>
                <span class="modal-price-value">${option.price}</span>
              </div>
            `
          )
          .join("")}
...
const renderDailyMenu = () => {
  ...
  dom.dailyMenuBody.innerHTML = `
    <h3 class="daily-menu-title">Menú del día</h3>
    <p class="daily-menu-subtitle"><span class="daily-menu-price">Menú Diario 8€ /// Medio Menú 5'50€</span></p>
    <p class="daily-menu-subtitle">De Lunes a Viernes, no festivos y al medio día.</p>
    ...
    <p class="daily-menu-note">No incluye bebida.</p>
  `;
};
