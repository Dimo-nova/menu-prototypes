...
const dailyMenuConfig = {
  primeros: ["empanada_atun", "pulpo_gallega", "croquetas_caseras"],
  segundos: ["brocheta_chipirones", "bacalao_hojaldre", "pollo_miel"],
};
        ...
        img:
          dish.image && (dish.image.startsWith("http") || dish.image.startsWith("images/"))
            ...
            : dish.image
              ...
              : "",
        video:
          dish.video && (dish.video.startsWith("http") || dish.video.startsWith("videos/"))
            ? dish.video
            : dish.video
              ? `../../data/${dish.video}`
              : "",
...
const renderSlider = () => {
  ...
  data.sliders.forEach((slide, index) => {
    ...
    if (hasImage && hasVideo) {
      // Si hay imagen y video, usar imagen como portada
      const img = document.createElement("img");
      ...
      // Crear video (oculto inicialmente)
      ...
      // Cuando el video termine, volver a mostrar la imagen
      ...
      // Cuando el video se pause, volver a mostrar la imagen
      ...
      // Click en el video para pausarlo
      ...
      // Cuando el video se pause, volver a mostrar el botón
      video.addEventListener("pause", () => {
        if (!video.ended) {
          playBtn.style.display = "flex";
          overlay.style.opacity = "1";
...
const openDishModal = (dish) => {
  ...
  if (hasImage || hasVideo) {
    if (hasImage && hasVideo) {
      ...
      videoContainer.className = "modal-media-slide";
      ...
      dom.dishModalMedia.appendChild(videoContainer);
    }
  } else {
